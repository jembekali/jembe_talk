...
class _MediaEditorViewState extends State<MediaEditorView> {
  ...
  void initState() {
    ...
    _textOverlays = widget.initialOverlays
            ?.map((e) => (e as MediaTextOverlay).copy())
            .toList() ??
        [];
          ...
          if (mounted) {
            setState(() {
              ...
              if (widget.initialEnd == null)
                _endValue = videoDur > 120 ? 120 / videoDur : 1.0;
            });
            _generateThumbnails();
  ...
  void _videoListener() {
    ...
    // Seek back if we hit the end trim
    if (vc.value.position >= vc.value.duration * _endValue) {
      vc.seekTo(vc.value.duration * _startValue);
    }

    // 🔥 Optimized: Update state only if playing status changed
    if (vc.value.isPlaying != _isPlayingPreview) {
  ...
  // 🔥 Optimized Thumbnail Generation (RAM Friendly)
  Future<void> _generateThumbnails() async {
    ...
    // Stop any ongoing FFmpeg sessions
    FFmpegKit.cancel();

    int count = 12; // Lower count for better RAM performance
    for (int i = 0; i < count; i++) {
      final time = (duration / count * i).toInt();
      final thumbPath = p.join(tempDir.path, 'thumb${const Uuid().v4()}.jpg');

      // 🔥 scale=160:-1 makes the thumbnail small to prevent OOM
      // executeAsync prevents blocking the UI
      await FFmpegKit.executeAsync(
          "-ss $time -i '${widget.file.path}' -vf scale=160:-1 -vframes 1 -q:v 15 '$thumbPath'",
          (session) async {
  ...
  @override
  void dispose() {
    FFmpegKit.cancel(); // Stop all processes
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
              ...
              onPressed: () {
                ...
                showDialog(
                    ...
                    builder: (c) => AlertDialog(
                            ...
                            title: Text(PostTranslations.t('add_text', l),
                            ...
                            actions: [
                              TextButton(
                                  ...
                                  child: Text(PostTranslations.t('cancel', l))),
                              ElevatedButton(
                                  onPressed: () {
                                    if (it.trim().isNotEmpty) {
                                      setState(() {
                                        _selectedOverlay = MediaTextOverlay(
                                            text: it.trim(),
                                            position: Offset(
                                                size.width / 2 - 80,
                                                size.height * 0.15));
          ...
          TextButton(
              onPressed: _confirmEdits,
              child: Text(PostTranslations.t('save_button', l),
                ...
                GestureDetector(
                  ...
                  onScaleUpdate: (details) {
                    if (_selectedOverlay == null) {
                      setState(() {
                        _zoomScale =
                            (_zoomScale * details.scale).clamp(1.0, 5.0);
                      ...
                      child: Container(
                          ...
                          child: ClipRRect(
                              ...
                              child: Stack(
                                  ...
                                  children: [
                                    Image.file(File(_selectedThumbnailPath!),
                                        width: 55,
                                        height: 80,
                                        fit: BoxFit.cover),
                                    Container(
                    ...
                    .map((o) => Positioned(
                          ...
                          child: GestureDetector(
                            onScaleUpdate: (d) => setState(() {
                              ...
                              _isOverDeleteZone =
                                  (o.position.dy > constraints.maxHeight - 150);
                            }),
  ...
  Widget _buildTimelineAndControls(String l, Size size) {
    ...
    double playheadX =
        (vc.value.position.inMilliseconds / (duration * 1000)) * totalWidth;
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(children: [
          IconButton(
              ...
              onPressed: () => _jumpToHandle(_startValue)),
          Expanded(
              child: Slider(
                  value: _timelineZoom,
                  min: 1.0,
          ...
          IconButton(
              ...
              onPressed: () => _jumpToHandle(_endValue)),
        ]),
        GestureDetector(
          onTapUp: (details) {
            ...
            vc.seekTo(
                Duration(milliseconds: (duration * seekRatio * 1000).toInt()));
          },
          child: SizedBox(
              height: 55,
                      ...
                      child: Stack(alignment: Alignment.centerLeft, children: [
                        Row(
                            children: _thumbnails
                                .map((t) => Image.file(File(t),
                                    width: totalWidth / _thumbnails.length,
                                    height: 40,
                                    fit: BoxFit.cover))
                                .toList()),
                        Positioned(
                            left: _startValue * totalWidth,
                            width: (_endValue - _startValue) * totalWidth,
                            child: Container(
                                height: 44,
                                decoration: BoxDecoration(
                            ...
                            child: GestureDetector(
                                onHorizontalDragUpdate: (d) {
                                  setState(() => _startValue =
                                      ((_startValue * totalWidth + d.delta.dx) /
                                              totalWidth)
                                          .clamp(0.0, _endValue - 0.001));
                                  vc.seekTo(Duration(
                                      milliseconds:
                                          (duration * _startValue * 1000)
                                              .toInt()));
                                },
                                child: _handleWidget(
                                    Colors.greenAccent, Icons.arrow_right))),
                        Positioned(
                            left: _endValue * totalWidth - 10,
                            child: GestureDetector(
                                onHorizontalDragUpdate: (d) {
                                  double maxEnd =
                                      _startValue + (120 / duration);
                                  setState(() => _endValue =
                                      ((_endValue * totalWidth + d.delta.dx) /
                                              totalWidth)
                                          .clamp(_startValue + 0.001,
                                              maxEnd.clamp(0.0, 1.0)));
                                  vc.seekTo(Duration(
                                      milliseconds:
                                          (duration * _endValue * 1000)
                                              .toInt()));
                                },
                                child: _handleWidget(
                                    Colors.redAccent, Icons.arrow_left))),
                      ])))),
        ),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text(_formatDur(Duration(seconds: (duration * _startValue).toInt())),
          ...
          Text(_formatDur(Duration(seconds: (duration * _endValue).toInt())),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          GestureDetector(
              ...
              child: const Row(children: [
                Icon(Icons.photo_library, size: 14, color: Colors.blueAccent),
                Text(" Gallery",
            ...
            child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                      onTap: () => setState(
                          () => _selectedThumbnailPath = _thumbnails[index]),
                      child: Container(
                          ...
                          child: Image.file(File(_thumbnails[index]),
          ...
          Container(
              ...
              child: ListView.builder(
                  ...
                  itemBuilder: (c, i) => GestureDetector(
                      onTap: () => setState(
                          () => _selectedOverlay!.color = _availableColors[i]),
                      child: Container(
                          width: 25,
                          height: 25,
        ...
        Row(children: [
          ...
          const Icon(Icons.palette, color: Colors.blueAccent, size: 16),
          Expanded(
              child: Slider(
                  value: _saturation,
                  min: 0.0,
  ...
  Widget _ratioBtn(String label, double? ratio) => GestureDetector(
      ...
      child: Container(
          ...
          decoration: BoxDecoration(
              color: _selectedAspectRatio == ratio
                  ? Colors.amberAccent
                  : Colors.white10,
  ...
  Widget _handleWidget(Color color, IconData icon) => Container(
      width: 20,
      height: 50,
      decoration:
  ...
  Widget _filterBtn(String label, String filterType) => GestureDetector(
      onTap: () => setState(() => _currentFilter = filterType),
      child: Container(
          margin: const EdgeInsets.only(right: 10),

...
import 'package:jembe_talk/language_provider.dart'; // <--- TWONGEREYEMWO IYI DOSIYE
import 'package:provider/provider.dart'; // <--- NA SATANI NI NGOMBWA
...
class _UnifiedNotificationsScreenState extends State<UnifiedNotificationsScreen> {
  ...
  Widget build(BuildContext context) {
    // Duhamagara LanguageProvider kugira dukoreshe indimi
    final lang = Provider.of<LanguageProvider>(context);
    final theme = Theme.of(context);
    final currentUser = FirebaseAuth.instance.currentUser;
    if (currentUser == null) return Scaffold(body: Center(child: Text(lang.t('notif_login_prompt')))); // <-- DUKORESHEJE 'lang.t()'

    return Scaffold(
      backgroundColor: theme.scaffoldBackgroundColor,
      appBar: AppBar(
        // Dukoresha 'lang.t()' no ku mutwe w'urupapuro
        title: Text("    ${lang.t('notif_title')}"), 
        backgroundColor: theme.appBarTheme.backgroundColor,
      ),
      body: ListView(
        children: [
          _buildFeedbackRepliesSection(context, currentUser.uid, lang), // Twongeyemwo 'lang'
          _buildAnnouncementsSection(context, lang), // Na hano nyene twongeyemwo 'lang'
        ],
      ),
    );
  }

  Widget _buildFeedbackRepliesSection(BuildContext context, String currentUserId, LanguageProvider lang) {
    final theme = Theme.of(context);
        ...
        return Column(
          ...
          children: [
            Padding(
              ...
              // Dukoresha 'lang.t()'
              child: Text(lang.t('notif_feedback_replies_header'), style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: theme.textTheme.bodyLarge?.color)),
            ),
            ...snapshot.data!.docs.map((doc) {
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  // Dukoresha 'lang.t()'
                  title: Text(lang.t('notif_reply_to_your_issue'), style: TextStyle(fontWeight: hasUnread ? FontWeight.bold : FontWeight.normal, color: theme.textTheme.bodyLarge?.color)),
  ...
  Widget _buildAnnouncementsSection(BuildContext context, LanguageProvider lang) {
    final theme = Theme.of(context);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
          ...
          // Dukoresha 'lang.t()'
          child: Text("      ${lang.t('notif_general_announcements_header')}", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: theme.textTheme.bodyLarge?.color)),
        ),
        StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            // Na hano nyene dukoresha 'lang.t()'
            if (!snapshot.hasData || snapshot.data!.docs.isEmpty) return Center(child: Padding(padding: EdgeInsets.all(16.0), child: Text(lang.t('notif_no_announcements'))));
            return Column(
              children: snapshot.data!.docs.map((doc) {
                final data = doc.data() as Map<String, dynamic>;
                // Na hano nyene, iyo ata mutwe uboneka
                final title = data['title'] ?? lang.t('notif_no_title');
                final message = data['message'] ?? lang.t('notif_no_message');

// lib/services/database_helper.dart (VERSION YUZUYE - WITH getMessageById)
...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 19) { try { await db.execute("ALTER TABLE $tableMessages ADD COLUMN $columnThumbnailLocalPath TEXT"); } catch (e) { log("Could not add thumbnailLocalPath column (maybe it exists?): $e"); } }
    if (oldVersion < 20) { try { await db.execute("ALTER TABLE $tableMessages ADD COLUMN $columnStoragePath TEXT"); } catch (e) { log("Could not add storagePath column (maybe it exists?): $e"); } }
    if (oldVersion < 21) { try { await db.execute("ALTER TABLE $tableMessages ADD COLUMN $columnThumbnailUrl TEXT"); } catch (e) { log("Could not add thumbnailUrl column (maybe it exists?): $e"); } }
    if (oldVersion < 22) { try { await db.execute("ALTER TABLE $tableJembeContacts ADD COLUMN $colBlockedUsers TEXT"); } catch (e) { log("Could not add blockedUsers column (maybe it exists?): $e"); } }
    if (oldVersion < 23) {
      try { await db.execute("ALTER TABLE $tableMessages ADD COLUMN $columnReplyingTo TEXT"); } catch (e) { log("Could not add replyingTo column (maybe it exists?): $e"); }
      try { await db.execute("ALTER TABLE $tableJembeContacts ADD COLUMN $columnLocalPhotoPath TEXT"); } catch (e) { log("Could not add localPhotoPath column (maybe it exists?): $e"); }
    }
    if (oldVersion < 24) {
      try { 
        await db.execute(''' CREATE TABLE $tableDeletedMessagesLog (
          $columnMessageId TEXT PRIMARY KEY, 
          $columnDeletedTimestamp INTEGER NOT NULL
        )''');
        log("Successfully created deleted_messages_log table."); 
      } catch (e) { log("Could not create deleted_messages_log table (maybe it exists?): $e"); }
    }
  }

  Future<void> _createTables(Database db) async {
    await db.execute(''' CREATE TABLE $tableMessages (
      $columnId TEXT PRIMARY KEY, $columnChatRoomID TEXT, $columnSenderID TEXT, $columnReceiverID TEXT, $columnMessageType TEXT, $columnMessage TEXT, $columnFileUrl TEXT, $columnOnlineUrl TEXT, $columnLocalPath TEXT, $columnFileName TEXT, $columnDuration INTEGER, $columnTimestamp INTEGER, $columnWaveform TEXT, $columnStatus TEXT, $columnThumbnailLocalPath TEXT, $columnStoragePath TEXT, $columnThumbnailUrl TEXT, $columnReplyingTo TEXT
    )''');
    ...
    await db.execute(''' CREATE TABLE $tableJembeContacts (
      $colUserId TEXT PRIMARY KEY, $colDisplayName TEXT, $colPhotoUrl TEXT, $columnLocalPhotoPath TEXT, $colPhoneNumber TEXT, $colEmail TEXT, $colBlockedUsers TEXT
    )''');
  ...
  Future<bool> saveMessage(Map<String, dynamic> messageData) async {
    ...
    final isDeleted = await isMessageDeleted(messageId);
    if (isDeleted) {
      log("Message $messageId is in the deleted log. Skipping save.");
      return false;
    }
  ...
  Future<void> clearChat(String chatRoomID) async {
    final db = await database;
    final messagesToDelete = await db.query(tableMessages, columns: [columnId], where: '$columnChatRoomID = ?', whereArgs: [chatRoomID]);
    ...
    for (var message in messagesToDelete) {
      ...
      batch.insert(tableDeletedMessagesLog, {
  ...
  Future<void> deleteMessage(String messageId) async {
    ...
    await batch.commit(noResult: true);
    log("Message $messageId deleted locally and added to deletion log.");
  }
  ...
  Future<void> cleanupDeletedMessagesLog() async {
    final db = await database;
    final cutoff = DateTime.now().subtract(const Duration(days: 4)).millisecondsSinceEpoch;
    final deletedCount = await db.delete(
      tableDeletedMessagesLog,
      where: '$columnDeletedTimestamp < ?',
      whereArgs: [cutoff],
    );
    if (deletedCount > 0) {
      log("Cleaned up $deletedCount old entries from the deleted messages log.");
    }
  }
  ...
  Future<void> updateMessageUrls(String messageId, {String? fileUrl, String? thumbnailUrl}) async {
    final db = await instance.database;
    Map<String, dynamic> dataToUpdate = {};
    if (fileUrl != null) dataToUpdate[columnFileUrl] = fileUrl;
    if (thumbnailUrl != null) dataToUpdate[columnThumbnailUrl] = thumbnailUrl;
    ...
    if (dataToUpdate.isNotEmpty) {
      ...
      log("DatabaseHelper: URLs for message $messageId updated.");
    }
  }
  ...
  // <<<--- IYI NIYO FUNCTION NSHASHA ONGEWEMO ---<<<
  ...
  Future<void> clearRegularPostsCache() async {
    ...
    log("Cache y'amaposita asanzwe yasivye neza.");
  }
  ...
  Future<void> _enforcePostLimit() async {
    ...
    if (count > _postCacheLimit) {
      ...
      final postsToDelete = await db.query(
        tablePosts,
        columns: [colPostId],
        where: '$colIsStar = 0',
      ...
      final idsToDelete = postsToDelete.map((row) => row[colPostId] as String).toList();
      if (idsToDelete.isNotEmpty) {
        await db.delete(
          tablePosts,
          where: '$colPostId IN (${List.filled(idsToDelete.length, '?').join(',')})',
  ...
  Future<List<Map<String, dynamic>>> getPosts({String? syncStatus, String order = 'DESC'}) async {
    final db = await instance.database;
    String orderBy = '$colTimestamp $order';
    if (syncStatus != null) {
      return await db.query(tablePosts, where: '$colSyncStatus = ?', whereArgs: [syncStatus], orderBy: orderBy);
    }
    return await db.query(tablePosts, orderBy: orderBy);
  }
  ...
  Future<void> saveComment(Map<String, dynamic> commentData) async {
    final db = await database;
    await db.insert(tableTangazaComments, commentData, conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<List<Map<String, dynamic>>> getCommentsForPost(String postId) async {
    final db = await database;
    return await db.query(tableTangazaComments, where: '$colPostId = ?', whereArgs: [postId], orderBy: '$colTimestamp ASC');
  }
  
  Future<void> toggleCommentLike(String commentId, String userId) async {
    final db = await database;
    final comment = await db.query(tableTangazaComments, where: '$colCommentId = ?', whereArgs: [commentId]);
    if (comment.isNotEmpty) {
      List<String> likedBy = (jsonDecode(comment.first[colLikedBy] as String? ?? '[]') as List).cast<String>();
      ...
      await db.update(tableTangazaComments, {
        colLikedBy: jsonEncode(likedBy),
        colLikesCount: likedBy.length
  ...
  Future<void> togglePostLikeStatus(String postId, bool isCurrentlyLiked) async {
    final db = await database;
    await db.update(tablePosts, {colIsLikedByMe: isCurrentlyLiked ? 1 : 0}, where: '$colPostId = ?', whereArgs: [postId]);
  }
  
  Future<void> incrementPostCommentCount(String postId) async {
    final db = await database;
    await db.rawUpdate('UPDATE $tablePosts SET $colCommentsCount = $colCommentsCount + 1 WHERE $colPostId = ?', [postId]);
  }
  
  Future<void> decrementPostCommentCount(String postId) async {
    final db = await database;
    await db.rawUpdate('UPDATE $tablePosts SET $colCommentsCount = $colCommentsCount - 1 WHERE $colPostId = ? AND $colCommentsCount > 0', [postId]);
  }

  Future<void> deleteComment(String commentId) async {
    final db = await database;
    await db.delete(tableTangazaComments, where: '$colCommentId = ?', whereArgs: [commentId]);
  }
  ...
  Future<void> updateComment(String commentId, String newText) async {
    final db = await database;
    await db.update(tableTangazaComments, {colText: newText}, where: '$colCommentId = ?', whereArgs: [commentId]);
  }
}
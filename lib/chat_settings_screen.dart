...
class _ChatSettingsScreenState extends State<ChatSettingsScreen> {
    ...
    try {
      ...
      if (pickedFile != null) {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(lang.t('chat_wallpaper_changed')), backgroundColor: Colors.green));
        }
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(lang.t('chat_wallpaper_error')), backgroundColor: Colors.redAccent));
      }
    }
  }
  ...
  void _showWallpaperOptions(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(lang.t('chat_wallpaper')), // "Ifoto y'Inyuma"
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ListTile(
                  leading: const Icon(Icons.photo_library_outlined),
                  title: Text(lang.t('btn_change_photo')), // "Hindura ifoto"
                  onTap: () {
              ...
              if (_wallpaperPath != null && _wallpaperPath!.isNotEmpty)
                ListTile(
                    ...
                    title: Text(lang.t('btn_remove_photo'), style: const TextStyle(color: Colors.redAccent)), // "Futa ifoto"
                    onTap: () {
                      Navigator.of(context).pop();
          ...
          actions: [
            TextButton(
              ...
              child: Text(lang.t('btn_cancel')), // "REKA"
            )
          ],
        );
      },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(lang.t('chat_title')), // "Ibiganiro"
        backgroundColor: theme.appBarTheme.backgroundColor,
      ),
      body: _isLoading ? const Center(child: CircularProgressIndicator()) : ListView(
        children: [
          _buildSectionHeader(context, lang.t('chat_display_header')), // "INGENE BIGARAGARA"
          _buildSettingsItem(
            context, 
            icon: Icons.color_lens_outlined, 
            title: lang.t('chat_theme'), 
            subtitle: lang.t('chat_theme_sub'), 
            onTap: () => Navigator.push(context, SlideRightPageRoute(page: const ThemeSettingsScreen()))
          ),
          _buildSettingsItem(
            context, 
            icon: Icons.wallpaper_outlined, 
            title: lang.t('chat_wallpaper'), 
            subtitle: lang.t('chat_wallpaper_sub'), 
            onTap: () => _showWallpaperOptions(context)
          ),
          
          const Divider(thickness: 0.5),
          _buildSectionHeader(context, lang.t('chat_settings_header')), // "INTUNGANYO Y'IBIGANIRO"
          
          SwitchListTile(
            title: Text(lang.t('chat_enter_send'), style: TextStyle(color: theme.textTheme.bodyLarge?.color)),
            subtitle: Text(lang.t('chat_enter_send_sub'), style: TextStyle(color: theme.textTheme.bodyMedium?.color)),
            value: _enterIsSend,
          ...
          _buildSettingsItem(
            ...
            title: lang.t('chat_font_size'), 
            subtitle: lang.t('chat_font_size_sub'),
